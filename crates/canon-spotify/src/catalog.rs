...
impl SpotifySession {
    ...
    /// What Spotify's `isrc:` search filter finds for this ISRC.
    async fn search_isrc(&self, isrc: &str) -> Result<Vec<SourceTrack>> {
...
impl Catalog for SpotifySession {
    ...
    /// The tracks Spotify has with this ISRC. The search filter is trusted only as far as the
    /// ISRC each result actually carries.
