...
impl RendererClock {
    ...
    /// A clock anchored at `start`, not yet running.
    ///
    ...
    /// A real discontinuity: move to `to`, preserving running/stopped.
    ...
    /// Fold in a position the renderer reported for itself.
    ///
    ...
    pub fn reconcile(&mut self, reported: Duration) -> Reconcile {
        let drift_ms = millis(reported) - millis(self.position());
...
mod tests {
    ...
    fn a_small_disagreement_slews_rather_than_jumping() {
        let mut clock = RendererClock::new(at(10_000));
    ...
    fn slewing_never_walks_the_position_below_zero() {
        let mut clock = RendererClock::new(at(10));
